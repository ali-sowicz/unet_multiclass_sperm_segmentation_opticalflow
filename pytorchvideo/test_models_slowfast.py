...
from models.functional import uniform_temporal_subsample_repeated
...
class TestSlowFast(unittest.TestCase):
    ...
    def test_create_slowfast_with_callable(self):
        ...
        for (norm, activation) in itertools.product(
            ...
            # Test forwarding.
            for tensor in TestSlowFast._get_inputs(
                input_channel, input_clip_length, input_crop_size, frame_ratios
            ):
                with torch.no_grad():
                    if tensor[0].shape[1] != input_channel:
                        with self.assertRaises(RuntimeError):
...
if __name__ == '__main__':
    t = TestSlowFast().test_create_slowfast_with_callable()